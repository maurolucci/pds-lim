...
MIPModel jovanovicModel(Pds &input) {
  ...
  for (auto v : boost::make_iterator_range(vertices(graph))) {
    ...
    // (3) x_v <= w_u_v + M(1-w_u_v), \forall v in V, u \in N(v)
    for (auto u : boost::make_iterator_range(adjacent_vertices(v, graph))) {
      GRBLinExpr constr3 = 0;
      constr3 += x.at(v) - w.at(std::make_pair(u, v)) - M * (1 - w.at(std::make_pair(u, v)));
      model.addConstr(constr3 <= 0);
    }

    // (4) x_v <= M(s_v + sum_{u \in N(v)} w_u_v + sum_{u \in N(v) \cap V_Z} y_u_v),
    // \forall v \in V
    ...
    if (input.isZeroInjection(v)) {
      for (auto u : boost::make_iterator_range(adjacent_vertices(v, graph))) {
        ...
        for (auto w : boost::make_iterator_range(adjacent_vertices(v, graph))) {
          ...
          constr8n += x.at(u) - x.at(w) - 1 + M * (1 - y.at(std::make_pair(v, u)));
          model.addConstr(constr8n >= 0);
        }
      }
    }

    // (9) sum_{u \in N(v)} w_v_u <= (omega_v - 1) s_v, \forall v \in V
